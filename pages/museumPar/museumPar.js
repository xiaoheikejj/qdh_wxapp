...
Page({
    ...
    onLoad: function (options) {
        ...
        wx.getImageInfo({
            // 文体场馆的业务类型是2
            src: `${app.baseUrl}/qdh-qrcode/2-${options.id}.png`,
            complete: res => {
    ...
    getMuseumDetail(id) {
        ...
        // 获取文体的地址
        const url = `${app.URL}/culture/info`;
        // 获取文体的参数
        const params = {
            cultureID: id,
        ...
        promise.then(res => {
            wx.hideLoading();
            if (res.resultCode != 1) {
                app.showError('获取场馆详情失败');
                return false
            }
            const museumResult = 'museum.result';
            this.setData({
                [museumResult]: res.module
            })
            const classifyActive = 'classify.active';
            // 视频图片全景
    ...
    /**收藏场馆 */
    collectMuseum() {
        const collection = 'museum.result.collection';
        // 改变collectStatus
        if (this.data.museum.result.collection === 1) {
            this.setData({
                [collection]: 0
            })
        ...
        // type = 5是文体
        if (this.data.museum.result.collection == 1) {
            favoriteCollect(collectUrl, 5, this.data.user.openID, this.data.museum.id, date).then(res => {
                if (res.resultCode != 1) {
                    return false
                } else {
                    this.getMuseumDetail(this.data.museum.id)
                }
            })
        } else {
            favoriteCancel(cancelUrl, 5, this.data.user.openID, this.data.museum.id).then(res => {
                if (res.resultCode != 1) {
                    return false
                } else {
                    this.getMuseumDetail(this.data.museum.id)
    ...
    dial() {
        wx.makePhoneCall({
            phoneNumber: this.data.museum.result.telno,
        })
    },
    ...
    orientation() {
        wx.openLocation({
            longitude: +this.data.museum.result.longitude,
            latitude: +this.data.museum.result.latitude,
            name: this.data.museum.result.spotsName,
            address: this.data.museum.result.address
        })
    },
    ...
    onShow: function () {
        // 获取文体详情
        this.getMuseumDetail(this.data.museum.id).then(res => {
    ...
    getPoster() {
        ...
        if (!this.data.museum.result.posterImg) {
            this.data.museum.result.posterImg = 'https://develop.magicreal.net/qdh-qrcode/posterImg.jpeg'
        }
        this.drawCanvas(this.data.museum.result.posterImg, this.data.museum.result.spotsName, this.data.museum.result.summary, this.data.museum.result.address);
        this.setData({
            [canvasshow]: true,
        })
    },
    ...
    getCodeMart() {
        ...
        const params = {
            url: 'pages/museumPar/museumPar',
            type: 2,
            id: this.data.museum.id
        };
        console.log(999)
        app.request(url, 'post', params).then(() => {
            wx.getImageInfo({
                // 美食的业务类型是5
                src: `${app.baseUrl}/qdh-qrcode/2-${this.data.museum.id}.png`,
                complete: res => {
                    if (res.path) {
                        this.setData({
                            wxCode: res.path
