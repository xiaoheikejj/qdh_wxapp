...
Page({
    ...
    onLoad: function (options) {
        ...
        wx.getImageInfo({
            src: `${app.baseUrl}/qdh-qrcode/3-${options.id}.png`,
            complete: res => {
    ...
    collectHotel() {
        ...
        // type = 3是酒店
        if (this.data.result.favorite == 1) {
            favoriteCollect(collectUrl, 3, this.data.user.openID, this.data.hotelID, date).then(res => {
                if (res.resultCode != 1) {
                    return false
                } else {
                    this.getHotelDetail(this.data.hotelID)
                }
            })
        } else {
            favoriteCancel(cancelUrl, 3, this.data.user.openID, this.data.hotelID).then(res => {
                if (res.resultCode != 1) {
                    return false
                } else {
                    this.getHotelDetail(this.data.hotelID)
    ...
    onShow: function () {
        // 获取酒店详情
        this.getHotelDetail(this.data.hotelID).then(res => {
    ...
    getCodeMart() {
        ...
        app.request(url, 'post', params).then(() => {
            wx.getImageInfo({
                // 美食的业务类型是5
                src: `${app.baseUrl}/qdh-qrcode/3-${this.data.hotelID}.png`,
                complete: res => {
                    if (res.path) {
                        this.setData({
                            wxCode: res.path
