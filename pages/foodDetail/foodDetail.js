...
Page({
    ...
    onLoad: function (options) {
        ...
        wx.getImageInfo({
            // 美食的业务类型是5
            src: `${app.baseUrl}/qdh-qrcode/5-${options.id}.png`,
            complete: res => {
    ...
    openMap() {
        wx.openLocation({
            ...
            name: this.data.result.cateName
        })
    },
    ...
    collectCate() {
        ...
        // type = 2美食
        if (this.data.result.favorite == 1) {
            favoriteCollect(collectUrl, 2, this.data.user.openID, this.data.cateID, date).then(res => {
                if (res.resultCode != 1) {
                    return false
                } else {
                    this.getRestaurantDetail(this.data.cateID)
                }
            })
        } else {
            favoriteCancel(cancelUrl, 2, this.data.user.openID, this.data.cateID).then(res => {
                if (res.resultCode != 1) {
                    return false
                } else {
                    this.getRestaurantDetail(this.data.cateID)
    ...
    onShow: function () {
        // 请求详情
        this.getRestaurantDetail(this.data.cateID).then(res => {
            wx.setNavigationBarTitle({
                title: res.module.cateName,
            })
            // 在当前页面隐藏导航条加载动画
    ...
    getPoster() {
        ...
        this.drawCanvas(this.data.result.posterImg, this.data.result.cateName, this.data.result.desc, this.data.result.address);
        this.setData({
            [canvasshow]: true,
        })
    },
    ...
    getCodeMart() {
        ...
        app.request(url, 'post', params).then(() => {
            wx.getImageInfo({
                // 美食的业务类型是5
                src: `${app.baseUrl}/qdh-qrcode/5-${this.data.cateID}.png`,
                complete: res => {
                    if (res.path) {
                        this.setData({
                            wxCode: res.path
